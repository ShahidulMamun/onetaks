...
<style>
    /* ── Primary: #006A4E ── */
    :root { --pr: #006A4E; --pr-dark: #004d38; --pr-light: #e6f4f0; --pr-border: #b2d8ce; }
    ...
    .step-item.active .step-dot { border-color: var(--pr); color: var(--pr); background: var(--pr-light); }
    .step-item.done   .step-dot { border-color: var(--pr); background: var(--pr); color: #fff; }
    .step-line { flex: 1; height: 1px; background: #e5e7eb; margin: 0 6px; }
    .step-line.done-line { background: var(--pr); }
    ...
    .opt-card:hover    { border-color: var(--pr-border); }
    .opt-card.selected { border-color: var(--pr); background: var(--pr-light); }
    ...
    .country-item:hover    { border-color: var(--pr-border); }
    .country-item.selected { border-color: var(--pr); background: var(--pr-light); color: var(--pr-dark); font-weight: 500; }
    ...
    .form-control:focus { border-color: var(--pr); background: #fff; }
    textarea.form-control { min-height: 90px; resize: vertical; }

    /* char counter */
    .char-hint { font-size: 11px; color: #9ca3af; margin-top: -10px; margin-bottom: 14px; display: block; }
    .char-hint.warn { color: #006A4E; font-weight: 500; }
    ...
    .thumb-area:hover { border-color: var(--pr); }
    ...
    .check-row input[type=checkbox] { width: 16px; height: 16px; accent-color: var(--pr); }
    ...
    .del-btn:hover:not(:disabled) { border-color: #fca5a5; color: #006A4E; background: #fef2f2; }
    ...
    .add-proof-btn:hover { border-color: var(--pr); color: var(--pr); }
    ...
    .btn-next { padding: 9px 24px; border: none; border-radius: 8px; background: var(--pr); color: #fff; font-size: 14px; font-weight: 500; cursor: pointer; }
    .btn-next:hover    { background: var(--pr-dark); }
    ...
    .budget-grid .b-val.red   { color: #006A4E; }
    .budget-total {
...
<form id="jobForm"
    ...
    <input type="hidden" name="secret_code_example" id="hSecretExample">
...
let form = {
    title: '', description: '',
    workerNeed: '',
    secretOn: false, secretCode: '', secretExample: ''
};
...
function renderStep3() {
    ...
    const remaining = TITLE_MAX - (form.title ? form.title.length : 0);
    ...
    document.getElementById('wizardContent').innerHTML = `
        ...
        <label class="form-label">
            Write an accurate job Title <span style="color:#006A4E">*</span>
            <span style="font-weight:400;font-size:11px;color:#9ca3af;">(max ${TITLE_MAX} characters)</span>
        </label>
        ...
        <span class="char-hint${remaining <= 5 ? ' warn' : ''}" id="charHint">
            ${remaining} character${remaining === 1 ? '' : 's'} remaining
        </span>

        <label class="form-label">Job description <span style="color:#006A4E">*</span></label>
        <textarea class="form-control" id="fDesc"
                  placeholder="Role requirements, responsibilities...">${escHtml(form.description)}</textarea>

        <label class="form-label">Number of workers needed <span style="color:#006A4E">*</span></label>
        ...
        <div id="secretBox" style="display:${form.secretOn ? 'block' : 'none'}">
            <label class="form-label">Secret code <span style="color:#006A4E">*</span></label>
            <input type="text" class="form-control" id="fSecret"
                   placeholder="Enter secret code"
                   value="${escHtml(form.secretCode)}">

            <label class="form-label">Secret code example
                <span style="font-weight:400;font-size:11px;color:red;">
                    (hint shown to workers)
                </span>
            </label>
            <input type="text" class="form-control" id="fSecretExample"
                   placeholder=''
                   value="${escHtml(form.secretExample)}">
        </div>

        <label class="form-label">Proof requirements <span style="color:#006A4E">*</span></label>
...
function proofRowHtml(p) {
    ...
    const placeholder = p.type === 'file'
        ? 'Image proof label e.g. Certificate photo'
        : 'Describe what text proof is needed';
    return `<div class="proof-block" id="proof_${p.id}">
        <div class="proof-row">
            <select onchange="updateProofType(${p.id}, this.value)">
                ...
                <option value="file"${p.type === 'file'  ? ' selected' : ''}>Image</option>
            </select>
            <input type="text"
                   id="proofLabel_${p.id}"
...
// FIX: type change হলে label save করে re-render করো
function updateProofType(id, val) {
    // label আগে save করো, তারপর type update করো
    const labelInput = document.getElementById(`proofLabel_${id}`);
    const proof = proofs.find(p => p.id === id);
    if (proof) {
        if (labelInput) proof.label = labelInput.value; // label preserve
        proof.type = val;
    }
    saveForm(); // বাকি সব field save
    renderStep3(); // re-render
}
...
function saveForm() {
    ...
    form.secretExample = document.getElementById('fSecretExample')?.value ?? form.secretExample;
    form.secretOn      = document.getElementById('secretCk')?.checked     ?? form.secretOn;

    // Save all proof labels from DOM
    proofs.forEach(p => {
        const inp = document.getElementById(`proofLabel_${p.id}`);
...
function submitJob() {
    ...
    if (form.title.trim().length > TITLE_MAX)        errs.push(`Job title must be ${TITLE_MAX} characters or fewer.`);
    ...
    document.getElementById('hSecretExample').value = form.secretOn ? form.secretExample : '';

    const proofsContainer = document.getElementById('hProofsContainer');
    proofsContainer.innerHTML = '';
    proofs.forEach((p, i) => {
        const t = document.createElement('input');
        t.type = 'hidden'; t.name = `proofs[${i}][type]`;  t.value = p.type;
        const l = document.createElement('input');
        l.type = 'hidden'; l.name = `proofs[${i}][label]`; l.value = p.label;
        proofsContainer.appendChild(t);
        proofsContainer.appendChild(l);
    });
